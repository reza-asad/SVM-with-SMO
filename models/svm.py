...
class SVM():
	...
	def naive_max_utility(self, X, y):
		...
		num_train  = len(X)
		self.params['alpha'] = np.zeors(num_train)[:,np.newaxis]
		idx = range(num_train)
		i = 0
		while i < 5:
			...
			a = -0.5 * (np.dot(x_m, x_m) + np.dot(x_n, x_n))
			...
			alpha_n = np.max(0, -float(b)/(2*a))
			alpha_m = np.max((si - alpha_n * y_n) * y_m, 0)
			self.params['alpha'][m] = alpha_m
			self.params['alpha'][n] = alpha_n
			print alpha_m, alpha_n
			i += 1
